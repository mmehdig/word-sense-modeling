...
abstract class MultiSenseEnrichedWordEmbeddingModel(val opts: EmbeddingOpts) extends Parameters {

  // Enrichment extension:
  class EnrichedVocabBuilder(vocab_hash_size: Int = 20e6.toInt, sampling_table_size: Int = 1e8.toInt, load_factor: Double = 0.7)
    extends VocabBuilder(vocab_hash_size, sampling_table_size, load_factor) {

    def addTokenToVocab(token: String): Unit = {
      val parts: Array[String] = token.split("__")

      if (parts.length >= 1) {
        addWordToVocab(parts.head.toLowerCase())
        if (parts.length > 1) {
          parts.tail.foreach(_.split('|').foreach(word => addWordToVocab(word.toLowerCase(), 1)))
        }
      }
  ...
  protected var vocab: EnrichedVocabBuilder   = null
  ...
  def buildVocab(minFreq: Int = 5): Unit = {
    vocab = new EnrichedVocabBuilder(vocabHashSize, samplingTableSize, 0.7) // 0.7 is the load factor
    ...
    if (loadVocabFilename.size == 0) {
      ...
      while (corpusLineItr.hasNext) {
        ...
        line.stripLineEnd.split(' ').foreach(token => vocab.addTokenToVocab(token))
        //        line.stripLineEnd.split(' ').foreach(w => vocab.addWordToVocab(w.split("__")(0).toLowerCase()))
      }
    }
    ...
    if (multiVocabFile.size != 0) {
      ...
      var n = 0
      (0 to vocab.size()).toStream.takeWhile(_ => n < learnTopV).foreach(
        v =>
          if (vocab.isContextual(v).equals(1)) {
