...
class MultiSenseEnrichedSkipGramEmbeddingModel(override val opts: EmbeddingOpts) extends MultiSenseEnrichedWordEmbeddingModel(opts) {

  class WordTranslationEmbed(token : String) {
    var tokens = token.split("__")
    var word : Int = -1
    var translationEmbed : Array[Int] = Array()

    if (tokens.size > 0)
      word = vocab.getId(tokens(0))
    if (tokens.size > 1)
      translationEmbed = tokens(1).split('|').map(w => vocab.getId(w)).filter(id => id != -1)
  }
  ...
  override def process(doc: String): Int = {
    ...
    var sen = doc.stripLineEnd.split(' ').map(word => new WordTranslationEmbed(word.toLowerCase())).filter(obj => obj.word != -1)
    ...
    //
    if (sample > 0)
      sen = sen.filter(obj => subSample(obj.word) != -1)

    val senLength = sen.size
    for (senPosition <- 0 until senLength) {
      val currWord = sen(senPosition).word
      ...
      // translations
      for (w <- sen(senPosition).translationEmbed)
        contexts += w
      ...
      contexts.foreach(context => {
        // postive example
        trainer.processExample(new EnrichedMSCBOWSkipGramNegSamplingExample(this, currWord, rightSense, context, 1))
        // for each POS example, make negative NEG examples. 
        //vocab.getRandWordId would get random word proportional (unigram-prob)^(0.75). 
        (0 until negative).foreach(neg => trainer.processExample(new EnrichedMSCBOWSkipGramNegSamplingExample(this, currWord, rightSense, vocab.getRandWordId, -1)))
    ...
    if (learnMultiVec(word)) {
      ...
      if (ncluster(word) < S) {
        if (createClusterlambda < minDist) {
          ...
          ncluster(word) += 1
          sense_weights(word) :+ Weights(TensorUtils.setToRandom1(new DenseTensor1(D, 0)))
          sense = ncluster(word)
        }
      }
    }
...
class EnrichedMSCBOWSkipGramNegSamplingExample(model: MultiSenseEnrichedWordEmbeddingModel, word: Int, sense : Int, context : Int, label: Int) extends Example {
